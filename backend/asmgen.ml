...
let cfg_unit_info = { Cfg_format.unit_name = ""; items = []; for_pack = None }
...
let emit_fundecl =
  if_emit_do (Profile.record ~accumulate:true "emit" Emit.fundecl)
...
let test_cfgize (f : Mach.fundecl) (res : Linear.fundecl) : unit =
  if ocamlcfg_verbose
  then Format.eprintf "processing function %s...\n%!" f.Mach.fun_name;
  let result =
  ...
  then
    Format.eprintf
      "the CFG on both code paths are equivalent for function %s.\n%!"
      f.Mach.fun_name
...
let compile_fundecl ~ppf_dump fd_cmm =
  ...
  ++ Profile.record ~accumulate:true "linearize" (fun (f : Mach.fundecl) ->
         ...
         if !Flambda_backend_flags.use_ocamlcfg then test_cfgize f res;
         res)
  ++ pass_dump_linear_if ppf_dump dump_linear "Linearized code"
  ++ Compiler_hooks.execute_and_pipe Compiler_hooks.Linear
  ++ (fun (fd : Linear.fundecl) ->
       if !Flambda_backend_flags.use_ocamlcfg
       then
         begin
           fd
...
let _compile_genfuns ~ppf_dump f =
  List.iter
    (function
...
let compile_unit ~output_prefix ~asm_filename ~keep_asm ~obj_filename gen =
  reset ();
  let create_asm = should_emit () && keep_asm in
...
let end_gen_implementation0 ?toplevel ~ppf_dump make_cmm =
  ...
  in
  (cmm @ genfuns @ [primitives])
  ++ Profile.record "compile_phrases" (Emit_llvm.implementation ~ppf_dump)
...
let compile_implementation_linear output_prefix ~progname =
  compile_unit ~output_prefix ~asm_filename:(asm_filename output_prefix)
    ~keep_asm:!keep_asm_file ~obj_filename:(output_prefix ^ ext_obj) (fun () ->
      (* TODO: remove *)
      if true then failwith "don't use linear";
